...
var updateBoard = function(board, click) {
    let crow = click[0], ccol = click[1]
    if (board[crow][ccol] === 'M') {
        board[crow][ccol] = 'X'
        return board
    }
    let height = board.length, width = board[0].length
    let inbound = function(row, col) {
        if (row < 0 || row >= height || col < 0 || col >= width) {
            return false
        }
        return true
    }
    let countMines = function(row, col) {
        let count = 0
        for (let x of [-1, 0, 1]) {
            for (let y of [-1, 0, 1]) {
                let nrow = row + x, ncol = col + y
                if (inbound(nrow, ncol) && board[nrow][ncol] === 'M') {
                    count++
                }
            }
        }
    ...
    let dfs = function(row, col) {
        if (!inbound(row, col) || board[row][col] !== 'E') {
            return
        }
