...
func canCross(stones []int) bool {
    deltas := []int{-1, 0, 1}
    smap := make(map[int]map[int]bool)
    for i := 0; i < len(stones); i++ {
        smap[stones[i]] = make(map[int]bool)
    }
    smap[0][0] = true
    for i := 0; i < len(stones) - 1; i++ {
        for k, _ := range smap[stones[i]] {
            for _, d := range deltas {
                if _, ok := smap[stones[i] + k + d]; ok {
                    smap[stones[i] + k + d][k + d] = true
                }
            }
        }
    }
    return len(smap[stones[len(stones) - 1]]) > 0
}
